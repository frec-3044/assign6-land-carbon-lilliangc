...
individual_final <- individual_final |> mutate(carbon = 0.5*(exp(B0 + B1*log(stemDiameter, base = exp(1)))))
...
individual_grouped <- individual_final |> 
  ...
  summarize(mean_carbon = mean(carbon, na.rm = TRUE)) |> 
  select(year, plotID, siteID, mean_carbon)
...
individual_plot <- 
  ...
  select(year, plotID, siteID, plotType, mean_carbon, totalSampledAreaTrees) |>
...
```{r}
individual_plot <-
  individual_plot |> mutate(carbonBYarea = mean_carbon/totalSampledAreaTrees)
